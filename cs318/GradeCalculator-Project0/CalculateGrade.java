...
public class CalculateGrade {
	...
	private static void readFile(String filename, double[] labs, double[] assts, double[] exams) {
		...
		try {
			...
			while((oneRecord = reader.readLine()) != null) {
				...
				//System.out.println(parsedRecord[0]);
				if (parsedRecord[0].equals("lab")) {
					labs[labPlace] = Double.parseDouble(parsedRecord[1]);
					//System.out.println(labs[labPlace]);
					labPlace++;
				}
				else if (parsedRecord[0].equals("asst")) {
					assts[asstPlace] = Double.parseDouble(parsedRecord[1]);
					//System.out.println(assts[asstPlace]);
					asstPlace++;
				}
				else {
					exams[examPlace] = Double.parseDouble(parsedRecord[1]);
					//System.out.println(exams[examPlace]);
					examPlace++;
				}
				++recordNumber;
			}
	...
	public static double calculateLabGrade100(double[] labs) {
		...
		for (int i = 0; i < labs.length; i++) {
			if (i < 4) {
				lab_total = lab_total + (labs[i] *.1);
			}
			else if (i >= 4) {
				lab_total = lab_total + (labs[i] *.15);
			}
		}
		return lab_total;
	}
	...
	public static double calculateAsstGrade100(double[] assts) {
		...
		for (int i = 0; i < assts.length; i++) {
			//System.out.println(assts[i]);

			if (i == 0) {
				asst_total = asst_total + (assts[i] *.1);
				//System.out.println(assts[i]);
			}
			else if (i >= 1 && i < 3) {
				asst_total = asst_total + (assts[i] *.16);
				//System.out.println(assts[i]);
			}
			else if (i >= 3 && i < 4) {
				asst_total = asst_total + (assts[i] *.18);
				//System.out.println(assts[i]);
			}
			else if (i >= 4 && i < 6) {
				asst_total = asst_total + (assts[i] *.2);
				//System.out.println(assts[i]);
			}
		}
		//System.out.println(asst_total);
		return asst_total;
	}
	...
	//TASK2b: Computed the scaled score for labs, assignments, midterm and final exam
	...
	public static double calculateScaledTotal(double[] exams, double lab_total, double asst_total) {
		//replace return value with the value you compute
		double exam_total = (.25*exams[0]) + (.3*exams[1]);
		double labScore = (lab_total/100)*15;
		double asstScore = (asst_total/100)*30;
		return exam_total+labScore+asstScore;
	}
	...
	public static void runTests(double[] labs, double[] assts, double[] exams) {
		...
		if(exams[1]!=95) {
			System.out.println("ERROR in Test 1b: Incorrect value in final.");
		...
		if (labs[1] != 90.0 || labs[2]!= 70.0) {
			...
			System.out.println("Expected to find a values of labs[1]=90.0 and labs[2]=70.0, but found labs[1]=" + labs[1] + " and labs[2]="+ labs[2] + " instead");
			allTestsPassed = false;
		}
		/////
		...
		if(assts[1] != 75.0 || assts[2]!= 80.0) {
			System.out.println("ERROR in Test 2b: Incorrect values read into labs array.");
			System.out.println("Expected to find a values of assts[1]=75.0 and assts[2]=80.0, but found labs[1]=" + assts[1] + " and labs[2]="+ assts[2] + " instead");
			allTestsPassed = false;
		}
		/////
